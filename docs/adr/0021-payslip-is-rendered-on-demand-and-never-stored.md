...
The dependency is pinned to an exact version in `Cargo.toml`, as every dependency here already is; a version bump is a deliberate, reviewed decision, exactly as the consequences section above already commits to keeping a renderer working.
