...
--
-- Existing Employers predate this column, so backfill them before enforcing
-- NOT NULL. There is no business name to recover from the old schema; a stable
-- placeholder keeps the upgrade lossless and visibly distinguishes legacy rows
-- until the Owner configuration capability can replace it.
--
...
ALTER TABLE employer ADD COLUMN name TEXT;
UPDATE employer SET name = 'Legacy Employer ' || id WHERE name IS NULL;
ALTER TABLE employer ALTER COLUMN name SET NOT NULL;

-- `btrim` covers spaces; the second part also closes PostgreSQL's `btrim`
-- gap for tabs and newlines.
ALTER TABLE employer
    ADD CONSTRAINT employer_name_is_not_blank
    CHECK (btrim(name) <> '' AND name ~ '[^[:space:]]');
